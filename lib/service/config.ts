export function getServiceConfig(env: Record<string, string | undefined> = {
  ...
  const deploymentOrigin = env.DEPLOY_URL;
  if (deploymentOrigin && !/^https:\/\/[a-f0-9]{24}--[a-z0-9-]+\.netlify\.app$/.test(deploymentOrigin)) throw new Error('SERVICE_CONFIG_INVALID');
  return { origin: origin.origin, functionUrl: functionUrl.href, ...(deploymentOrigin ? { deploymentOrigin } : {}) };
}
