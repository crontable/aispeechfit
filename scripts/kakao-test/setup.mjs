...
const missing = Object.entries(defaults).sort(([a], [b]) => a.localeCompare(b)).filter(([key]) => !(key in existing));
if (missing.length) appendFileSync(path, '\n# 카카오 개발 앱 연동 검증 전용\n' + missing.map(([key, value]) => `${key}=${value}`).join('\n') + '\n', { mode: 0o600 });
chmodSync(path, 0o600);
console.log('개발 전용 설정 준비 완료. 기존 환경 변수는 유지했습니다.');
console.log('.env.local의 KAKAO_CLIENT_ID와 KAKAO_CLIENT_SECRET 값을 입력하세요.');
