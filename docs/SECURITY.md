...
현재 브랜치의 카카오 전환 코드는 `docs/SERVICE-AUTH-ROLLOUT.md`를 따른다. 아래 내용은 운영에 적용된 #26 기준이며, 새 인증의 추가 정책은 로컬 검사만 완료했고 운영에는 아직 적용하지 않았다. 서비스 페이지는 새 세션 검사로 변경했고, 원격 데이터 연결이 준비되지 않으면 접근을 허용하지 않는다.
