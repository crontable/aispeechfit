...
서비스 연결 구현 현황과 운영 연결 조건은 `docs/SERVICE-AUTH-ROLLOUT.md`를 따른다. 현재 브랜치는 Google 버튼을 제거하고 서비스 페이지의 세션 검사를 Better Auth로 교체했다. 원격 데이터 연결·본인 이용권·운영 적용은 미완료다. 아래 앞선 단계의 검증 기록과 구분한다.
