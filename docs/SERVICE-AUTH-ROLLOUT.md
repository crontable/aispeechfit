...
Session pooler로 수정한 뒤 서버 연결과 공식 CA를 사용한 TLS 검증이 통과했다. 조사 도구에 공식 공개 CA를 포함했다. 현재는 DB가 비밀번호 인증 실패(`28P01`)를 반환하여 원본 조회 전 단계에서 중단된다. 사용자가 DB 비밀번호와 URL 인코딩을 확인한 뒤 재실행한다. 이전 IPv6 주소 문제는 해소되었다.
