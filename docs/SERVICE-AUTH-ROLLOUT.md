...
전환 SQL에는 `app.ticket_source_id`, `app.ticket_target_user_id`, `app.kakao_app_id`를 동일 DB 연결의 설정으로 전달한다. 실제 원격 카카오 사용자 검증 후 값을 확정한다. 새 인증 스키마와 세션 접근 함수가 먼저 있어야 한다. 원격 배포 도구는 각 파일의 BEGIN/COMMIT 경계를 정리하여 정책·이용권 전환을 하나의 트랜잭션으로 적용해야 하며, 운영 역할과 서명 키 검증 전 개별 적용하지 않는다. 현재는 SQL 및 로컬 검증 완료 단계이고 원격 배포 도구·원격 적용은 미완료다.
